...
//! `CRAFT_TRUECOLOR=1` or `=0` overrides everything.
...
fn truecolor_from_env(get: impl Fn(&str) -> Option<String>) -> Option<bool> {
    match get("CRAFT_TRUECOLOR").as_deref() {
        Some("1" | "true") => return Some(true),
        Some("0" | "false") => return Some(false),
        Some(other) => tracing::warn!(value = other, "ignoring invalid CRAFT_TRUECOLOR"),
        None => {}
    }
    let has =
...
mod tests {
    ...
    #[test_case(&[("CRAFT_TRUECOLOR", "1")], Some(true); "override_forces_truecolor")]
    #[test_case(&[("CRAFT_TRUECOLOR", "0"), ("COLORTERM", "truecolor")], Some(false); "override_forces_downgrade")]
    #[test_case(&[("CRAFT_TRUECOLOR", "true")], Some(true); "override_word_true")]
    #[test_case(&[("CRAFT_TRUECOLOR", "false"), ("COLORTERM", "truecolor")], Some(false); "override_word_false")]
    #[test_case(&[("CRAFT_TRUECOLOR", "off")], None; "invalid_override_ignored")]
    #[test_case(&[("CRAFT_TRUECOLOR", "off"), ("COLORTERM", "truecolor")], Some(true); "invalid_override_falls_through")]
