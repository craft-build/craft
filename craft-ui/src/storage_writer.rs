...
type Pending = Arc<Mutex<HashMap<CraftId, Entry>>>;
...
impl StorageWriter {
    pub fn new(dir: StateDir, warn_tx: Option<flume::Sender<String>>) -> Result<Self> {
    ...
    pub fn send(&self, session: Arc<AppSession>) {
        self.enqueue(session.id.id(), Entry::Save(session));
    }
    ...
    fn enqueue(&self, id: CraftId, entry: Entry) {
...
struct Writer {
    dir: StateDir,
    warn_tx: Option<flume::Sender<String>>,
    /// Only cursors that still describe their file.
    logs: HashMap<CraftId, SessionLog>,
    /// Sessions whose last write failed, so a sick disk warns once instead of
    /// once per frame.
    failing: HashSet<CraftId>,
}

impl Writer {
    fn forget(&mut self, id: CraftId) {
        self.logs.remove(&id);
    ...
    fn write(&mut self, session: &AppSession) -> Result<(), SessionError> {
        ...
        if let Some(mut log) = self.logs.remove(&session.id.id()) {
            ...
            if !matches!(appended, Err(SessionError::LogDiverged { .. })) {
                self.logs.insert(session.id.id(), log);
                return appended;
            }
        }
        ...
        self.logs.insert(
            session.id.id(),
            SessionLog::rewrite(&sessions_dir, session)?,
        );
        Ok(())
    }

    fn report(&mut self, id: CraftId, result: Result<(), impl std::fmt::Display>) {
        match result {
            Ok(()) => {
                if self.failing.remove(&id) {
                    self.send_warn(SAVE_RECOVERED.to_string());
                }
            }
            Err(e) => {
                warn!(error = %e, %id, "session write failed");
                if self.failing.insert(id) {
                    self.send_warn(format!("{SAVE_FAILED_PREFIX}: {e}"));
                }
            }
        }
    }

    fn send_warn(&self, msg: String) {
        if let Some(tx) = &self.warn_tx {
            let _ = tx.send(msg);
        }
    }
}
...
mod tests {
    ...
    fn writer(dir: &StateDir) -> (StorageWriter, flume::Receiver<String>) {
        ...
        (
            StorageWriter::new(dir.clone(), Some(warn_tx)).unwrap(),
            warn_rx,
        )
    }
    ...
    fn user_message(n: usize) -> craft_providers::Message {
        craft_providers::Message::user(msg_text(n))
    }
    ...
    fn drain_warns(rx: &flume::Receiver<String>) -> Vec<String> {
        rx.drain().collect()
    }
    ...
    fn reopened_log_rewrites_diverged_file_instead_of_appending() {
        ...
        match loaded.tool_outputs().get(TOOL_ID).map(Arc::as_ref) {
            Some(craft_agent::ToolOutput::Plain(out)) => assert_eq!(out, TOOL_TEXT),
            other => panic!("tool output lost: {other:?}"),
        }
        assert!(drain_warns(&second_warn_rx).is_empty());
    }
    ...
    fn failing_flush_warns_once_and_reports_recovery() {
        ...
        writer.send(Arc::clone(&session));
        let (done_tx, done_rx) = flume::bounded(1);
        writer.delete(CraftId::generate(), move |res| {
            let _ = done_tx.send(res);
        });
    ...
    fn save_enqueued_after_delete_survives() {
        ...
        assert!(drain_warns(&warn_rx).is_empty());
    }
