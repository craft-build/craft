...
pub fn highlight_line(hl: &mut craft_highlight::Highlighter, text: &str) -> Vec<Span<'static>> {
...
pub fn highlight_regex_inline(pattern: &str) -> Vec<Span<'static>> {
    let Some(syntax) = craft_highlight::syntax_set().find_syntax_by_token("re") else {
        return vec![fallback_span(pattern)];
    };
    let mut hl = craft_highlight::Highlighter::for_syntax(syntax);
    highlight_line(&mut hl, pattern)
}

pub fn fallback_span(text: &str) -> Span<'static> {
    Span::styled(
        craft_highlight::normalize_text(text),
