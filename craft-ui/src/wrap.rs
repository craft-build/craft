...
//! `Wrap { trim: false }` case craft always uses, so row counts match what
...
/// ratatui reports nothing, because that is what craft's layout expects.
///
...
impl Measure {
    ...
    pub fn rows(&mut self, line: &Line<'_>) -> u16 {
        self.scan
            .as_mut()
