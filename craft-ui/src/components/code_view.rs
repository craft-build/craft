...
fn highlight_spans(hl: &mut craft_highlight::Highlighter, text: &str) -> Vec<Span<'static>> {
...
struct FileWalker<'a> {
    ...
    pos: usize,
    hl: craft_highlight::Highlighter,
}
...
impl<'a> FileWalker<'a> {
    fn new(content: &'a str, syntax: &'static SyntaxReference) -> Self {
        Self {
            ...
            pos: 1,
            hl: craft_highlight::Highlighter::for_syntax(syntax),
        }
    }
    ...
    match syntax {
        ...
        None => {
            ...
            spans.push(Span::styled(
                craft_highlight::normalize_text(&full),
                base.patch(theme::current().code_block),
            ));
        }
    }
...
fn render_grep_results(
    ...
    for entry in entries {
        ...
        let syntax = highlight.then(|| craft_highlight::syntax_for_path(&entry.path));
        ...
        for (gi, group) in entry.groups.iter().enumerate() {
            ...
            for line in &group.lines {
                ...
                let text_spans = if let Some(syn) = syntax {
                    highlight_spans(
                        &mut craft_highlight::Highlighter::for_syntax(syn),
                        &line.text,
                    )
...
pub fn render_tool_content(
    ...
    let (output_lines, output_trunc) = match output {
        ...
        Some(ToolOutput::Diff {
            ...
            render_diff(
                highlight.then(|| craft_highlight::syntax_for_path(path)),
                before,
                after,
            ),
            false,
...
mod tests {
    ...
    fn fg_in_context(path: &str, prefix: &str, text: &str, find: &str) -> ratatui::style::Color {
        let mut hl = craft_highlight::Highlighter::for_path(path);
    ...
    fn diff_context_line_inside_block_comment_matches_full_file_state() {
        ...
        let lines = render_diff(
            Some(craft_highlight::syntax_for_path("test.rs")),
            before,
            after,
        );
