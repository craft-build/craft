...
impl CommandPalette {
    ...
    /// Name lookup for `craft.api.run_command`, returning the registered
...
mod tests {
    use super::*;
    use craft_agent::{McpPromptArg, McpSnapshot};
    ...
    fn sample_custom() -> Arc<[CustomCommand]> {
        Arc::from([
            CustomCommand {
                ...
                scope: craft_agent::command::CommandScope::Project,
                accepts_args: true,
            },
            CustomCommand {
                name: "fix".into(),
                ...
                scope: craft_agent::command::CommandScope::User,
                accepts_args: false,
            },
        ])
    }
    ...
    fn lua_commands_update_on_generation_change() {
        let (writer, reader) = craft_lua::test_support::lua_command_writer_pair();
