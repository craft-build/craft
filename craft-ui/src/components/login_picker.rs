...
impl LoginPicker {
            ...
            Step::EnterKey {
                ...
                KeyCode::Enter => {
                    ...
                    if let Some(model) = &default_model {
                        let _ = persist_model(&storage, model);
                    }
    ...
    pub fn view(&mut self, frame: &mut Frame, area: Rect) -> Rect {
        match &mut self.step {
            ...
            Step::CustomName { input } => render_input_modal(
                frame,
                area,
                " Provider name ",
            ...
            Step::CustomUrl { input, .. } => render_input_modal(
                frame,
                area,
                " Base URL ",
            ...
            Step::EnterKey {
                ...
                render_input_modal(frame, area, &format!(" {display_name} "), hint_text, input)
            }
            Step::Done { message } => {
                let modal = Modal {
                    title: TITLE,
...
mod tests {
    ...
    fn picker_with_storage() -> (LoginPicker, TempDir) {
        let dir = TempDir::new().unwrap();
        let storage = craft_storage::StateDir::from_path(dir.path().to_path_buf());
    ...
    #[test]
    fn go_custom_name_sets_custom_name_step() {
        let (mut picker, _dir) = picker_with_storage();
        let action = picker.transition(StepAction::GoCustomName);
        assert!(matches!(action, LoginPickerAction::Consumed));
        assert!(matches!(picker.step, Step::CustomName { .. }));
    }

    #[test]
    fn back_from_custom_name_returns_to_pick_provider() {
        let (mut picker, _dir) = picker_with_storage();
        picker.transition(StepAction::GoCustomName);
        let action = picker.transition(StepAction::Back);
        assert!(matches!(action, LoginPickerAction::Consumed));
        assert!(matches!(picker.step, Step::PickProvider(..)));
    }

    #[test]
    fn go_pick_plan_sets_pick_plan_step() {
        let (mut picker, _dir) = picker_with_storage();
        let action = picker.transition(StepAction::GoPickPlan {
            slug: "mistral".to_string(),
        });
        assert!(matches!(action, LoginPickerAction::Consumed));
        assert!(matches!(picker.step, Step::PickPlan { .. }));
    }

    #[test]
    fn go_builtin_url_sets_builtin_url_step() {
        let (mut picker, _dir) = picker_with_storage();
        let action = picker.transition(StepAction::GoBuiltinUrl {
            slug: "ollama".to_string(),
            display_name: "Ollama".to_string(),
        });
        assert!(matches!(action, LoginPickerAction::Consumed));
        assert!(matches!(picker.step, Step::BuiltinUrl { .. }));
    }

    #[test]
    fn go_custom_url_sets_custom_url_step() {
        let (mut picker, _dir) = picker_with_storage();
        let action = picker.transition(StepAction::GoCustomUrl {
            slug: "mycustom".to_string(),
            protocol: "openai".to_string(),
        });
        assert!(matches!(action, LoginPickerAction::Consumed));
        assert!(matches!(picker.step, Step::CustomUrl { .. }));
    }

    #[test]
    fn empty_input_in_custom_name_is_rejected() {
        let (mut picker, _dir) = picker_with_storage();
        picker.transition(StepAction::GoCustomName);
    ...
    #[test]
    fn empty_api_key_in_enter_key_is_rejected() {
        let (mut picker, _dir) = picker_with_storage();
        ...
        let action = picker.handle_key(key(KeyCode::Enter));
        assert!(matches!(action, LoginPickerAction::Consumed));
        assert!(matches!(picker.step, Step::EnterKey { .. }));
    }

    #[test_case(Some("openai/gpt-4.1".to_string()), "openai".to_string() ; "with_model_returns_authenticated")]
    #[test_case(None, "ollama".to_string() ; "without_model_returns_configured")]
    fn go_done_returns_correct_action(model_spec: Option<String>, slug: String) {
        let (mut picker, _dir) = picker_with_storage();
        let has_model = model_spec.is_some();
        let action = picker.transition(StepAction::GoDone {
            message: "test".to_string(),
            model_spec,
            slug,
        });
        assert!(matches!(picker.step, Step::Done { .. }));
        if has_model {
            assert!(matches!(action, LoginPickerAction::Authenticated { .. }));
        } else {
            assert!(matches!(action, LoginPickerAction::Configured { .. }));
        }
    }
}
