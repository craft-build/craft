...
fn search_text_includes_role_prefix() {
    ...
    assert_eq!(texts[2], format!("● {md}"));
    assert_eq!(texts[4], "○ hmm");
}
...
fn batch_start(panel: &mut MessagesPanel, entries: Vec<BatchToolEntry>) {
    panel.tool_start(ToolStartEvent {
        ...
        input: None,
        raw_input: None,
        output: Some(Arc::new(ToolOutput::Batch {
            entries,
            text: String::new(),
            no_compress: false,
        })),
...
fn batch_done(panel: &mut MessagesPanel, _entries: Vec<BatchToolEntry>) {
    panel.tool_done(ToolDoneEvent {
        id: "b1".into(),
        tool: "batch".into(),
        output: Arc::new(ToolOutput::Batch {
            entries: vec![],
            text: String::new(),
            no_compress: false,
        }),
...
fn scroll_clamps_to_max_scroll() {
    ...
    panel.streaming_text.set_buffer(&"a\n".repeat(15));
    render(&mut panel, 80, 10);
    let bottom = panel.layout().bottom(panel.viewport_height);

    panel.scroll(-3);
    assert_eq!(panel.scroll, bottom);
}
...
fn extract_partial_column_selection() {
    ...
    let world_start = CRAFT_PREFIX_LEN + "Hello ".len() as u16;
...
fn extract_mixed_fully_enclosed_and_partial() {
    ...
    let area = Rect::new(0, 0, 80, 24);
    let sel = make_sel(&panel, area, (0, 0), (seg1_start, CRAFT_PREFIX_LEN + 6));
    let text = panel.extract_selection_text(&sel, area);
...
fn extract_partial_col_symmetric(msgs: &[&str], expect_start: &str, expect_end: &str) {
    ...
    let total = panel.layout().total_rows();
    let area = Rect::new(0, 0, 80, 24);
    let down = make_sel(&panel, area, (0, CRAFT_PREFIX_LEN), (total - 1, 79));
    let up = make_sel(&panel, area, (total - 1, 79), (0, CRAFT_PREFIX_LEN));
...
fn panel_with_long_tool(line_count: usize) -> MessagesPanel {
    ...
    panel.tool_done(ToolDoneEvent {
        id: "t1".into(),
        tool: BASH_TOOL_NAME.into(),
        output: Arc::new(ToolOutput::Plain(body)),
        is_error: false,
        annotation: None,
        written_path: None,
    });
...
fn search_text_includes_truncated_bash_output() {
    ...
    panel.tool_done(ToolDoneEvent {
        id: "t1".into(),
        tool: BASH_TOOL_NAME.into(),
        output: Arc::new(ToolOutput::Plain(full_output.clone())),
        is_error: false,
        annotation: None,
        written_path: None,
    });
...
fn panel_with_snapshot_tools(count: usize) -> MessagesPanel {
    ...
    for i in 0..count {
        ...
        panel.tool_snapshot(
            &id,
            BufferSnapshot::from_arc(std::sync::Arc::new(vec![snap_line(&format!("out {i}"))])),
            None,
        );
    }
    panel
...
fn selection_survives_a_resize() {
    ...
    let mut sel = Selection::start(
        first_row_of(&panel, ALPHA, CRAFT_PREFIX_LEN),
        wide,
        SelectionZone::Messages,
    );
