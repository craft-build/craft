...
/// A `test_app` past its idle splash, whose drifting wiring would mask
/// every other cadence.
...
fn submit_prompt_never_interprets_text(text: &str) {
    ...
    match app.submit_prompt(queued_msg(text)) {
        SubmitOutcome::Started(actions) => {
            assert!(matches!(actions.first(), Some(Action::SendMessage(_))))
        }
...
fn streaming_app_without_queue() -> App {
    let mut app = test_app();
    app.status = Status::Streaming;
    app.queue = MessageQueue::default();
    app
}
...
fn edge_scroll_direction(zone: Rect, down: (u16, u16), drag: (u16, u16), expected: Option<i32>) {
    let mut app = app_with_transcript(zone);
...
fn tick_drains_live_bufs_of_background_chats() {
    ...
    assert_owes_one_frame(&mut app, || {
        buf.append(craft_agent::SnapshotLine::plain(TOOL_OUTPUT_LINE.into()));
    });
}
...
fn open_overlay_motion_reaches_app_cadence() {
    ...
    app.float_mgr.open(
        Arc::new(craft_agent::SharedBuf::new()),
...
fn set_opus_model(app: &mut App) {
    app.state.model = craft_providers::Model::from_spec(OPUS_SPEC).unwrap();
}
...
fn model_state_reports_the_model_and_what_it_supports() {
    let mut app = test_app();
    app.state.model = craft_providers::Model::from_spec(PLAIN_MODEL_SPEC).unwrap();
    assert_eq!(
        app.model_state(),
        serde_json::json!({
...
fn model_change_fires_once_per_real_swap() {
    ...
    app.update_model(&craft_providers::Model::from_spec(OPUS_SPEC).unwrap());
    app.emit_model_change();
    app.emit_model_change();
...
/// `craft.model.get` -> `craft.model.set` hop would silently change it.
...
fn fast_turns_off_on_a_model_that_lost_fast_support() {
    ...
    app.state.model = craft_providers::Model::from_spec(SONNET_SPEC).unwrap();
    app.execute_command(cmd("/fast"), 0);
    assert!(!app.state.fast);
    assert_eq!(app.status_bar.flash_text(), Some(FAST_OFF_MSG));
}
...
fn update_model_to_ineligible_resets_fast() {
    ...
    let sonnet = craft_providers::Model::from_spec(SONNET_SPEC).unwrap();
...
) -> craft_lua::test_support::RequestProbe {
    app.keymap_reader = craft_lua::test_support::keymap_reader_with(vec![craft_lua::KeymapEntry {
        key,
        modifiers,
