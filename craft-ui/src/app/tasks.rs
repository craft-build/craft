...
//! session's own and `craft.session.live()` already reports that.
//!
//! Both `craft.task.list()` and the `TaskStatusChanged` autocmd serialize the
...
/// The wire shape of `craft.task.list()`. The main chat leaves `status` unset.
#[derive(Serialize)]
