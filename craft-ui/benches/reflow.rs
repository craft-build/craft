...
fn blocks_only(texts: &[String]) -> usize {
    texts.iter().map(|t| craft_markdown::parse(t).len()).sum()
}
...
fn bench_highlight(c: &mut Criterion) {
    ...
    g.bench_function("parse_lines_only", |b| {
        b.iter(|| {
            let mut ps = ParseState::new(craft_highlight::syntax_for_token("rust"));
            for line in CODE_BLOCK.lines() {
                let _ = ps.parse_line(line, craft_highlight::syntax_set());
            }
        })
    });
    for (name, theme) in [
        ...
        craft_highlight::set_theme(theme);
        g.bench_function(format!("code_highlighter_update_{name}"), |b| {
            b.iter(|| {
                craft_highlight::CodeHighlighter::new("rust")
...
fn bench_reflow(c: &mut Criterion) {
    ...
    let lines: Vec<Line<'static>> = texts
        .iter()
        .flat_map(|t| text_to_lines(t, "craft> ", Style::default(), Style::default(), WIDE, None))
        .collect();
