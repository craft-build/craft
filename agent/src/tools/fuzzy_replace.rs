//! Fuzzy-matched find/replace used by the edit-family tools.
//!
//! Ports the reference engine: candidate strings are produced by a ladder of
//! increasingly forgiving passes (exact → line-trimmed → block-anchor →
//! whitespace-normalized → indentation-flexible → NFKD → trimmed-boundary →
//! context-aware → escape-normalized), and the first pass that yields a
//! usable match wins. When a fuzzy pass forgave indentation drift, the
//! replacement is rebased into the file's indentation frame.

use regex::Regex;
use unicode_normalization::UnicodeNormalization;

pub(crate) const NO_MATCH: &str = "old_string not found in file";
pub(crate) const EMPTY_OLD_STRING: &str = "old_string cannot be empty";
pub(crate) const MULTIPLE_MATCHES: &str = "old_string matches multiple locations; add surrounding context to make it unique, or use occurrence param to select which match";
pub(crate) const OCCURRENCE_OUT_OF_RANGE: &str = "occurrence number exceeds number of matches";
...
pub(crate) fn replace(
    ...
    replace_inner(content, old_string, new_string, replace_all, occurrence)
}
