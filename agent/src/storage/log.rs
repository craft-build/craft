//! Size-capped rotating log files (200 MB × 10 by default).
//!
//! Ported from the reference `craft-storage/src/log.rs`. Deviation: the
//! reference takes its cross-process rotation lock with the `fs4` crate;
//! that crate is not a dependency here, so the same advisory flock is taken
//! via the stabilized `std::fs::File::lock`.
...
fn file_path(dir: &Path, index: u32) -> PathBuf {
    if index == 0 {
        ...
        dir.join(format!("craft.{index}.log"))
    }
}

/// Exclusive lock guarding log rotation across processes: an advisory flock
/// on the lock file (`std::fs::File::lock`), released on drop. The lock file
/// itself is never deleted, so a crashed process cannot strand a stale
/// marker; the kernel drops its flock with its file descriptors.
struct RotationLock {
    file: Option<File>,
}
...
impl RotationLock {
    fn acquire(dir: &Path) -> Self {
        ...
        match file {
            // Blocking flock: waits for any concurrent rotator to finish.
            Ok(f) if f.lock().is_ok() => Self { file: Some(f) },
            _ => Self { file: None },
        }
    }
}
...
impl RotatingFileWriter {
    ...
    fn rotate(&mut self) -> io::Result<()> {
        ...
        let _lock = RotationLock::acquire(&self.dir);
            ...
            for i in (0..last).rev() {
                ...
                if src.exists() {
                    ...
                    if let Err(e) = fs::rename(&src, &dst) {
                        eprintln!(
                            "craft: log rotate rename {} -> {} failed: {e}",
                            src.display(),
                            dst.display()
                        );
                    }
...
mod tests {
    ...
    #[test]
    fn rotation_lock_is_released_after_drop() {
        let tmp = tempfile::tempdir().unwrap();
        ...
        // The next acquire must succeed immediately.
        let lock = RotationLock::acquire(tmp.path());
        assert!(lock.file.is_some());
    }
}
