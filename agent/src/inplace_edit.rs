...
//! (reference: `craft-agent/src/agent/inplace_edit.rs`).
//!
...
/// Detect files rewritten in place by `sed -i` / `perl -i` leaves of a
/// compound command. Fails open (empty) on any ambiguity.
...
mod tests {
    ...
    #[test]
    fn sed_detects_in_place_variants() {
        assert_eq!(
            paths("sed -i 's/a/b/' file.txt"),
            vec![FILE_TXT.to_string()]
        );
        assert_eq!(
        ...
        assert_eq!(
            paths("sed --in-place=.bak 's/a/b/' file.txt"),
            vec![FILE_TXT.to_string()]
        );
        assert_eq!(
            paths("sed -i -e 's/a/b/' file.txt"),
            vec![FILE_TXT.to_string()]
        );
        assert_eq!(
            paths("sed -i -e 's/a/b/' -e 's/c/d/' file.txt"),
            vec![FILE_TXT.to_string()]
        );
        assert_eq!(
            paths("sed -i -e 's/a/b/' -e 's/c/d/' one.txt two.txt"),
            vec![ONE_TXT.to_string(), TWO_TXT.to_string()]
        );
        assert_eq!(
            paths("sed -i 's/a/b/' one.txt two.txt"),
            vec![ONE_TXT.to_string(), TWO_TXT.to_string()]
        );
        assert_eq!(
            paths("sed -i 's/a/b/' file.txt file.txt"),
            vec![FILE_TXT.to_string()],
            "repeated file arg deduped"
        );
        assert_eq!(
            paths("sed -i 's/a/b/' -- file.txt"),
            vec![FILE_TXT.to_string()]
    ...
    #[test]
    fn perl_detects_in_place_variants() {
        assert_eq!(
            paths("perl -i -pe 's/a/b/' file.txt"),
            vec![FILE_TXT.to_string()]
        );
        assert_eq!(
            paths("perl -i.bak -pe 's/a/b/' file.txt"),
            vec![FILE_TXT.to_string()]
        );
        assert_eq!(
            paths("perl -ipe 's/a/b/' file.txt"),
            vec![FILE_TXT.to_string()]
        );
        assert_eq!(
            paths("perl -i -e 's/a/b/' -e 's/c/d/' file.txt"),
            vec![FILE_TXT.to_string()]
        );
    }
