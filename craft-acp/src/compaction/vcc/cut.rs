...
fn is_prompt_user(msg: &Message) -> bool {
    matches!(
        msg,
        Message::User { content } if content.iter().any(|b| matches!(b, UserContent::Text(_)))
    )
}
...
mod tests {
    ...
    fn cut_keeps_tail_after_last_user_prompt() {
        ...
        assert!(matches!(&history[cut.tail_start], Message::User { content }
            if matches!(&content[0], UserContent::Text(t) if t.text == "do task two")));
    }
    ...
    fn cut_returns_none_for_short_history() {
        let history = vec![user("hi"), assistant_tool("t1", "bash")];
    ...
    fn cut_mid_cycle_for_single_prompt_chain() {
        ...
        for i in 0..10 {
            history.push(assistant_tool(&format!("t{i}"), "bash"));
