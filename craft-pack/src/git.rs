//! Every git invocation craft makes.
//!
...
/// both. Git records the URL in `.git/config`, and craft also records the source
...
///
/// `craft.pack.add` runs while `init.lua` is being sourced, and `init.lua`
