...
impl ProvidersConfig {
    ...
    pub fn save(&self) -> Result<(), std::io::Error> {
        ...
        #[cfg(unix)]
        fs::set_permissions(&path, fs::Permissions::from_mode(PROVIDERS_FILE_MODE))?;
...
pub fn ignored_builtin_fields(slug: &str, def: &ProviderDef) -> Vec<&'static str> {
    ...
    if def.models.as_ref().is_some_and(|m| !m.is_empty()) {
        ignored.push("models");
    }
...
"#
    ...
    fn resolve_base_url_prefers_def_over_none() {
        ...
        let slug = "craft-test-def-over-none-slug";
        let def = ProviderDef {
    ...
    fn resolve_base_url_empty_def_matches_none() {
        let slug = "craft-test-empty-def-slug";
    ...
    fn resolve_base_url_custom_slug_uses_def() {
        let slug = "craft-test-custom-base-url-slug";
        let def = ProviderDef {
    ...
    fn resolve_base_url_env_beats_def() {
        let slug = "craft-test-env-base-url-slug";
