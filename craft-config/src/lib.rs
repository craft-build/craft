...
/// the package approval store, and named on every guarded `craft.*` function.
///
/// It lives here rather than in `craft-lua` so the tool layer can name the
/// permission a tool exposes without pulling in the lua runtime.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, strum::VariantArray)]
pub enum Permission {
    FsRead,
    FsWrite,
    Net,
...
impl Permission {
    ...
    pub const ALL: &'static [Permission] = <Permission as strum::VariantArray>::VARIANTS;
    ...
    pub const fn describes(self) -> &'static str {
        match self {
            Permission::FsRead => "reading files, and locating the directories craft keeps them in",
...
"#;
    ...
    fn expand_env_expands_whole_value_var() {
        unsafe { std::env::set_var("CRAFT_TEST_HDR_WHOLE_71535", "secret") };
        assert_eq!(
            expand_env("${CRAFT_TEST_HDR_WHOLE_71535}").as_deref(),
            Ok("secret")
        );
    }
    ...
    fn expand_env_expands_mid_string_var() {
        unsafe { std::env::set_var("CRAFT_TEST_HDR_MID_71535", "tok") };
        assert_eq!(
            expand_env("Bearer ${CRAFT_TEST_HDR_MID_71535}!").as_deref(),
            Ok("Bearer tok!")
        );
    }
    ...
    fn expand_env_expands_multiple_vars() {
        unsafe { std::env::set_var("CRAFT_TEST_HDR_A_71535", "a") };
        unsafe { std::env::set_var("CRAFT_TEST_HDR_B_71535", "b") };
        assert_eq!(
            expand_env("${CRAFT_TEST_HDR_A_71535}-${CRAFT_TEST_HDR_B_71535}").as_deref(),
            Ok("a-b")
        );
    }
    ...
    fn expand_env_unset_var_names_the_variable() {
        assert_eq!(
            expand_env("Bearer ${CRAFT_TEST_HDR_UNSET_71535}"),
            Err("CRAFT_TEST_HDR_UNSET_71535".to_string())
        );
    }
    ...
    fn expand_env_empty_var_is_treated_as_unset() {
        unsafe { std::env::set_var("CRAFT_TEST_HDR_EMPTY_71535", "") };
        assert_eq!(
            expand_env("Bearer ${CRAFT_TEST_HDR_EMPTY_71535}"),
            Err("CRAFT_TEST_HDR_EMPTY_71535".to_string())
        );
    }
}
