...
fn login_provider(slug: &str, storage: &StateDir) -> Result<()> {
    ...
    if has_key {
        println!("  Credentials: {}", credential_path(slug));
    } else {
        let env_var = resolve_api_key_env(slug, config.get(slug));
        println!(
            "  Set API key via: {} or run: craft auth login {}",
            env_var, slug
        );
    }
...
fn login_custom(storage: &StateDir) -> Result<()> {
    ...
    println!("  Use with: craft -m {}/<model>", slug);
...
fn select_plan(
    slug: &str,
    builtin: Option<&'static craft_config::providers::BuiltInProvider>,
...
pub fn prompt(
    ...
    use craft_agent::agent::{build_system_prompt, load_instruction_text};
    use craft_agent::prompt::{PromptId, assemble};
    use craft_agent::template;
    use craft_agent::tools::{DescriptionContext, ToolFilter};
    ...
    let output = match variant {
        PromptVariant::System => {
            let mode = if plan {
                AgentMode::Plan(std::path::PathBuf::from("plan.md"))
            } else {
                AgentMode::Build
            };
