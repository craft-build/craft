        ...
        match outcome {
            ...
            RunOutcome::Reload {
                ...
                let pack_report = if let Some(plan) = pack {
                    teardown.join();
                    drop(stack);
                    Some(craft_lua::apply_pack_plan(plan))
                } else {
                    teardown.defer(move || drop(stack));
                    None
                };
...
mod tests {
    ...
    fn teardown_defer_joins_previous_and_drop_joins_last() {
        ...
        let saw = Arc::clone(&second_saw_first);
        teardown.defer(move || {
            saw.store(first_done.load(Ordering::Acquire), Ordering::Release);
        });
        drop(teardown);
    ...
    fn no_plugins_skips_broken_init_lua_but_keeps_host_alive() {
        ...
        let craft_dir: PathBuf = dir.path().join(".craft");
        fs::create_dir_all(&craft_dir).expect("mkdir .craft");
        fs::write(
            craft_dir.join("init.lua"),
            "error('broken init lua must not run')",
        )
        .expect("write init.lua");

        let cli = Cli::parse_from(["craft", "--no-plugins"]);
