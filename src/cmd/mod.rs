...
use craft_lua::{DiscoveredPackage, Interaction, PluginHost};
...
/// `craft.pack.add`. Resolved by `build_config` instead of handed to it, because
...
pub(crate) fn load_plugins(
    ...
    // Installing here rather than inside `craft.pack.add` keeps a clone off the
    // Lua thread, and is the phase Neovim's own `load` default defers to.
    let declared = declared_packages(host)?;
    let installed = craft_lua::install_declared(&declared, interaction);
        ...
        Some(Command::Auth { action }) => {
            ...
            match action {
                AuthAction::Login { provider } => {
                    subcmd::auth_login(provider.as_deref(), &storage).await?
                }
