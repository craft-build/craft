...
use craft_storage::input_history::MAX_ENTRIES;
...
fn move_logs(legacy: &Path, logs_dir: &Path) -> Result<()> {
    let entries: Vec<_> = fs::read_dir(legacy)
        ...
        .filter(|e| {
            ...
            name.starts_with("craft.") && name.ends_with(".log")
        })
        .collect();
...
pub fn xdg() -> Result<()> {
    ...
    println!("Moving files from ~/.craft/ ...\n");
    ...
    let lock_file = legacy.join("craft.log.lock");
    if lock_file.exists() {
    ...
    let backup = legacy.with_file_name(".craft.bak");
    if has_leftovers {
        fs::create_dir_all(&backup).context("create ~/.craft.bak/")?;
        for name in &remaining {
            let src = legacy.join(name);
            let dst = backup.join(name);
            if dst.exists() {
                println!("  {name} (skipped, already in ~/.craft.bak/)");
            } else if let Err(e) = fs::rename(&src, &dst) {
                eprintln!("  warning: could not move {name} to ~/.craft.bak/: {e}");
            }
        }
    }

    if legacy.is_dir() {
        fs::remove_dir_all(&legacy).context("remove ~/.craft/")?;
    }
    ...
    println!(
        "\nAll done! Your files now live here:\n\n\
         ...
         \x20 Logs     {}\n\n\
         Per-project settings (.craft/ in your repos) are not affected.\n\n\
         Removed ~/.craft/.",
        tilde(&xdg.config),
        tilde(&xdg.state),
        tilde(&xdg.logs),
