//! SDK streaming mode: `craft --print --input-format stream-json`.
//!
...
fn craft_to_claude_tool_name(name: &str) -> &str {
    TOOL_NAME_MAP
        .iter()
        .find(|(m, _)| *m == name)
        .map(|(_, c)| *c)
...
pub async fn run(params: SdkParams) -> Result<()> {
    let SdkParams {
        cli,
        model,
        mut config,
    ...
    let tools: Vec<&str> = handle
        .tool_names
        .iter()
        .map(|t| craft_to_claude_tool_name(t))
        .collect();
    writer.emit_system(
        "init",
        serde_json::json!({
            ...
            AgentEvent::ToolStart(ts) => {
                ...
                if self.include_partial_messages {
                    ...
                    let events = self.synth.tool_use(
                        &model,
                        &ts.id,
                        craft_to_claude_tool_name(&name),
                ...
                self.writer
                    .emit(WireInner::ControlRequest(ControlRequestPayload {
                        ...
                        request: ControlRequestInner {
                            ...
                            tool_name: Some(craft_to_claude_tool_name(&tool_name).into()),
                            input: Some(input),
            ...
            let mapped: Vec<Value> = blocks
                ...
                .map(|block| {
                        ...
                        && let Some(name) = block.get("name").and_then(Value::as_str)
                    {
                        ...
                        b["name"] = Value::String(craft_to_claude_tool_name(name).to_string());
                        return b;
                    }
                    block.clone()
                })
...
mod tests {
    ...
    fn claude_to_craft_tool_name(name: &str) -> &str {
        TOOL_NAME_MAP
            .iter()
            .find(|(_, c)| *c == name)
            .map(|(m, _)| *m)
    ...
    fn craft_to_claude_roundtrip(craft: &str, claude: &str) {
        assert_eq!(craft_to_claude_tool_name(craft), claude);
        assert_eq!(claude_to_craft_tool_name(claude), craft);
    }

    #[test]
    fn unknown_tool_name_passthrough() {
        assert_eq!(craft_to_claude_tool_name("unknown_tool"), "unknown_tool");
        assert_eq!(claude_to_craft_tool_name("UnknownTool"), "UnknownTool");
    }
