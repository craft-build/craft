...
use craft_storage::{StateDir, atomic_write};
use tracing::warn;
...
pub fn provider_info<T: Any + Send + Sync + 'static>(
    provider: &str,
    model_id: &str,
) -> Option<Arc<T>> {
    let info = read()
...
mod tests {
    ...
    fn curated_provider_ignores_discovered_tiers() {
        ...
        assert_ne!(
            reg.tier_for("synthetic/syn:discovered:strong", "synthetic", None),
            ModelTier::Strong
        );
        assert_ne!(
            reg.spec_for_tier("synthetic", ModelTier::Strong),
            Some("synthetic/syn:discovered:strong".into())
        );
    }
    ...
    fn discovered_looks_up_by_id() {
        ...
        reg.set_known_models("llama-cpp", vec![info_a, info_b]);
