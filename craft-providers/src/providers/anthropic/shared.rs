...
static EMPTY_CONTENT: LazyLock<Value> =
    LazyLock::new(|| json!({"type": "text", "text": EMPTY_RESPONSE_MARKER}));
...
fn wire_content(msg: &Message) -> Vec<WireContentBlock> {
    let mut content: Vec<WireContentBlock> = msg
        .content
        .iter()
    ...
    messages
        ...
        .map(|(msg_idx, msg)| {
            ...
            if msg_idx + MESSAGE_CACHE_BREAKPOINTS >= len
                && let Some(block) = content.iter_mut().rfind(|b| {
                    !b.inner
                        .get("type")
