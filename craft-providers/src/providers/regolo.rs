...
/// `/model_group/info` in [`Regolo::list_models_with_info`]. Context windows
/// mirror the group's `max_input_tokens`, the same field [`join_model_info`]
/// prefers, not its `max_tokens` (input plus output) which would let a session
/// grow past what the upstream accepts.
...
impl Provider for Regolo {
    ...
    async fn list_models_with_info(&self) -> Result<Vec<ModelInfo>, AgentError> {
        let auth = lock_unpoison(&self.auth).clone();
        let ids = self.compat.do_list_models(&auth).await?;
        ...
        match groups {
            ...
            None => Ok(ids.into_iter().map(ModelInfo::new).collect()),
        }
    }
