...
use craft_config::providers::{OverrideFields, ProviderOverride};
use flume::Sender;
...
fn load_overrides() -> Overrides {
    let overrides = craft_config::providers::ProvidersConfig::load()
        .get("aperture")
...
/// hit its responses-API path and bypass the gateway. Copilot, xAI, and
/// Bedrock are excluded: their auth is not a clean proxy target.
...
/// Path prefix craft sends to the gateway, which appends the whole incoming
...
fn resolve_base_url() -> Result<String, AgentError> {
    ...
    if let Some(url) = craft_config::providers::ProvidersConfig::load()
        .get("aperture")
...
/// (so `aperture/mistral/glm-5-2` picks up the real `context_window` without
    ...
    if let Some(kind) = routed_kind(provider_id, &ov) {
        ...
        if let Some(manifest) = manifest_for_kind(kind) {
            ...
            if let Ok(entry) = lookup_entry(manifest.models, model_id) {
                ...
                model.supports_vision_override = model
                    .supports_vision_override
                    .or(Some(entry.supports_vision));
            }
        }
    }
...
impl Provider for Aperture {
    ...
    fn adjust_model(&self, model: &mut Model) {
        if let Some((provider_id, model_id)) = model.id.split_once('/') {
            ...
            let ov = merged_override(&self.overrides, provider_id, &model_id);
            if let Some(kind) = routed_kind(provider_id, &ov) {
                let auth = routed_auth(&self.auth, &path_prefix(Some(kind), &ov));
                if let Ok(routed) =
                    build_routed_provider(kind, auth, self.timeouts, self.system_prefix.clone())
                {
...
mod tests {
    ...
    #[test_case("tensorx", Some(ProviderKind::TensorX) ; "known_tensorx")]
    ...
    #[test_case("xai", None ; "xai_excluded")]
    #[test_case("bedrock", None ; "bedrock_excluded")]
    ...
    #[test_case("tensorx", "not-a-real-provider", Some(ProviderKind::TensorX) ; "invalid_base_falls_back_to_provider_id")]
    ...
    #[test_case(ProviderKind::TensorX, "aperture/tensorx/z-ai/glm-5.2", "tensorx/z-ai/glm-5.2" ; "compat_tensorx_keeps_vendor_prefix")]
    ...
    #[test_case(Some(ProviderKind::TensorX), Some("https://aperture.example.com/v1") ; "tensorx_appends_v1")]
    ...
    fn merged_override_model_wins_over_provider_default() {
        ...
        let ov = merged_override(&overrides, "tensorx", "z-ai/glm-5.2");
    ...
    fn apply_adjustments_uses_routed_provider_static_table() {
        let mut model = Model::from_spec("aperture/deepseek/deepseek-v4-flash").unwrap();
        assert_eq!(
    ...
    fn apply_adjustments_vision_override_enables_gateway_model() {
        ...
        let mut model = Model::from_spec("aperture/synthetic/hf:MiniMaxAI/MiniMax-M3").unwrap();
        apply_adjustments(&mut model, &overrides);
        assert_eq!(model.supports_vision_override, Some(true));
        assert!(model.supports_vision());
    }
    ...
    #[test_case("aperture/deepseek/deepseek-v4-flash", ProviderKind::DeepSeek ; "routed_thinking_capable")]
    #[test_case("aperture/ollama/qwen3", ProviderKind::Ollama ; "routed_non_thinking")]
    #[test_case("aperture/mistral/glm-5-2", ProviderKind::Mistral ; "routed_mistral")]
...
[aperture.overrides.tensorx]
context_window = 128000
max_output_tokens = 8192
path_prefix = ""

[aperture.overrides.tensorx.models."z-ai/glm-5.2"]
context_window = 200000
supports_thinking = true
supports_vision = true
"#;
        let config: craft_config::providers::ProvidersConfig = toml::from_str(toml).unwrap();
        ...
        assert_eq!(path_prefix(Some(ProviderKind::TensorX), &ov), "");
        let ov2 = merged_override(&overrides, "tensorx", "other");
