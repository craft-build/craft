...
/// `info` and `models` describe the script, not the world, so their
/// output only changes when the script does. Caching them keeps two
/// process spawns per provider off every startup.
#[derive(Serialize, Deserialize, PartialEq, Eq, Clone)]
struct ScriptDescription {
    modified_ns: u128,
    size: u64,
    info: String,
    /// `None` when the script has no `models` subcommand, or its run
    /// failed.
...
/// Reuses `cached` for as long as the file stays untouched. A failing
/// `info` is fatal for the provider, while `models` is optional and a
/// failure there is cached as `None`. A script we cannot stat gets a
/// zero stamp, which never matches, so it is described again on
/// every run.
fn describe_script(
    slug: &str,
    path: &Path,
...
/// Bad `models` output is survivable, the base provider's model list
/// takes over. Anything else the script gets wrong drops it, with a
/// line in the log.
fn build_meta(
    slug: String,
    script_path: PathBuf,
...
fn write_cache(cache: &ScriptCache) {
    ...
    if let Err(e) = craft_storage::atomic_write(&path, &bytes) {
...
fn discover_in(dir: &Path) -> Vec<DynamicProviderMeta> {
    ...
    for entry in entries.flatten() {
        ...
        // Cached even when the description was rejected, so a script we
        // refuse is not re-run on the next startup either.
...
pub fn lookup_model(slug: &str, model_id: &str) -> Option<Model> {
    ...
    Some(Model {
        ...
        thinking_fields: script_model.thinking_fields.clone().map(Box::new),
    })
}
...
impl Provider for DynamicProvider {
    ...
    async fn reload_auth(&self) -> Result<(), AgentError> {
        ...
        run_auth_script(self.script_path, &self.auth, "reload").await
    }
...
mod tests {
    ...
    fn script_model_deserialization() {
        ...
        let resolved = Model {
            id: model.id.clone(),
            provider: Arc::from("dynamic"),
            tier: model.tier,
            family: ProviderKind::LlamaCpp.family(),
            supports_tool_examples_override: model.supports_tool_examples,
            thinking_override: None,
            supports_vision_override: None,
            pricing: model.pricing.clone().unwrap_or_default(),
            max_output_tokens: Some(model.max_output_tokens),
            context_window: model.context_window,
            thinking_fields: model.thinking_fields.clone().map(Box::new),
        };
...
"#;
    ...
    #[tokio::test]
    async fn refresh_gate_single_flights_concurrent_callers(rotating: bool, final_token: &str) {
        let tmp = TempDir::new().unwrap();
        ...
        let (first, second) =
            tokio::join!(gate.refresh(&script, &auth), gate.refresh(&script, &auth));
        first.unwrap();
        second.unwrap();
