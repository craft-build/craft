...
impl Model {
    ...
    pub fn is_free(&self) -> bool {
        self.discovered_free
            || crate::providers::opencode::free_model_if_available(&self.provider, &self.id)
    }
}
...
mod tests {
    ...
    fn sum_marker_applies_only_to_the_cost() {
        ...
        assert_eq!(
            usage.format_sum_cost(Some(1.5)),
            "12.3k\u{2191} 456\u{2193} Σ$1.500"
        );
    ...
    #[test_case("synthetic/hf:moonshotai/Kimi-K3", true ; "synthetic_hf_vision_alias")]
    ...
    #[test_case("claude-opus-4-8",  true  ; "opus_4_8_still_fast")]
    ...
    fn discovered_pricing_decides_free(pricing: Option<ModelPricing>, expected: bool) {
        ...
        crate::model_registry::set_known_models(
            ...
            vec![ModelInfo {
                pricing,
                ..ModelInfo::new(model_id.to_string())
            }],
        );
