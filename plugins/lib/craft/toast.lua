-- Corner toast notifications built on floating windows. `craft.ui.flash` gives
...
-- Above the default every other float gets, so a picker opened after the
...
function Toast.show(text, opts)
  ...
  for line in tostring(text):gmatch("[^\n]+") do
    ...
    width = math.max(width, craft.ui.display_width(line) + 2)
  end
  if #lines == 0 then
    return
  end
  ...
  local buf = craft.ui.buf({ scratch = true })
  buf:set_lines(lines)
