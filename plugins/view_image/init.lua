...
local MAX_EDGE = 1568
-- Refuse absurdly large files up front; craft.image.decode also enforces a
...
local function load_image(path)
  local bytes, read_err = craft.fs.read_bytes(path)
  if not bytes then
  ...
  local info, probe_err = craft.image.probe(bytes)
  if not info then
  ...
  local img, decode_err = craft.image.decode(bytes)
  if not img then
  ...
  if size <= MAX_RAW_BYTES and math.max(info.width, info.height) <= MAX_EDGE then
    return {
      ...
      image = { media_type = media_type, data = craft.base64.encode(bytes) },
    }
  end
  ...
  return {
    ...
    image = { media_type = MEDIA_TYPES[out_format], data = craft.base64.encode(encoded) },
  }
end
...
craft.api.register_tool({
  ...
  handler = function(input, _ctx)
    ...
    local path = craft.fs.abspath(raw)
