...
local function with_mock_ui(fn)
  local original = { buf = craft.ui.buf, markdown = craft.ui.markdown }
  ...
  craft.ui.buf = function()
    return buf
  end
  craft.ui.markdown = function(text, _width)
    return { { { text, "" } } }
  end
  local ok, err = pcall(fn, buf)
  craft.ui.buf = original.buf
  craft.ui.markdown = original.markdown
  if not ok then
    error(err)
  end
end
...
local function line_width(line)
  ...
  for _, span in ipairs(line) do
    w = w + craft.ui.display_width(span[1])
  end
  return w
end
...
case("wrap_spans_hard_splits_oversize_word_on_valid_utf8_boundaries", function()
  for _, c in ipairs({ { word = "abcdefghij", width = 4 }, { word = "éééééé", width = 3 } }) do
...
case("truncate_text_returns_empty_head_when_narrower_than_glyph", function()
  local t = craft.ui.truncate_text("你好", 1)
  eq(t.head, "")
  eq(t.tail, "你好")
  local t2 = craft.ui.truncate_text("你a", 1)
  eq(t2.head, "")
  eq(t2.tail, "你a")
end)

case("truncate_text_zero_width_returns_empty", function()
  local t = craft.ui.truncate_text("abc", 0)
  eq(t.head, "")
  eq(t.tail, "abc")
  local empty = craft.ui.truncate_text("", 5)
  eq(empty.head, "")
  eq(empty.tail, "")
end)
