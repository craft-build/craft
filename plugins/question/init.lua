...
craft.api.register_tool({
  ...
  -- Craft does not persist a per-result `state` table across sessions, so a
  -- restored question renders its saved llm_output through markdown instead of
  -- rebuilding the interactive card. Diverges from upstream, which keeps the
  -- picks in `state` and rebuilds the card via `ctx:state()`.
  restore = function(input, output, is_error, ctx)
    return ToolView.restore_markdown(output, is_error, QuestionHelpers.view_opts(ctx))
  end,
})
