...
local function display_width(s)
  return craft.ui.display_width(s)
end
...
local function split_at(s, max_cols)
  ...
  local t = craft.ui.truncate_text(s, max_cols)
...
local function question_md(state, idx, width)
  ...
  local ok, lines = pcall(craft.ui.markdown, text, width)
...
function QuestionForm.open(questions)
  ...
  local max_h = math.floor(craft.ui.terminal_size().rows * MAX_HEIGHT_RATIO)
  ...
  while true do
    ...
    if ev.type == "resize" then
      width = ev.width
      max_h = math.floor(craft.ui.terminal_size().rows * MAX_HEIGHT_RATIO)
