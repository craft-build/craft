...
local function icon_of(s)
  if s.status == "needs_input" then
    return "\xe2\x97\x86 ", "warning" -- "◆ "
  end
  if s.status == "working" then
    return WORKING_ICON, "accent"
  end
  if s.focused then
    return "\xe2\x97\x8f ", "accent" -- "● "
  end
  if s.live then
    return "\xe2\x97\x8b ", "accent" -- "○ "
  end
  return "  ", "dim"
end
...
local function update_footer()
  ...
  if board.counts.needs_input > 0 then
    footer[#footer + 1] = { "\xe2\x97\x86 " .. board.counts.needs_input, "needs input" }
  end
  if board.counts.working > 0 then
    footer[#footer + 1] = { "\xe2\x97\x8f " .. board.counts.working, "working" }
  end
...
local function render()
  ...
  local words = filter_words()
  for _, s in ipairs(board.items) do
    local selected = s.id == board.sel_id
    local icon, icon_style = icon_of(s)
    ...
    for _, sp in ipairs(ListPicker.highlight_spans(s.title, words, base, match_style)) do
      line[#line + 1] = sp
    end
...
local function refresh()
  ...
  local live, live_err = craft.session.live()
  if board ~= this_board then
    return
  end
  if live_err then
    craft.ui.flash(live_err)
    render()
    return
  end
  local seen, all = {}, {}
...
local function open_selected()
  ...
  if not s.focused then
    local _, err = craft.session.focus(s.id)
    if err then
      craft.ui.flash(err)
      return
    end
...
local function open_blank()
  local _, err = craft.session.new({ focus = true })
  if err then
    craft.ui.flash(err)
    return
  end
...
local function delete_selected()
  ...
  if s.focused then
    craft.ui.flash(DELETE_FOCUSED_HINT)
    return
  end
  if board.confirm ~= s.id then
    board.confirm = s.id
  ...
  board.confirm = nil
  local _, err = craft.session.delete(s.id)
  if err then
    craft.ui.flash(err)
    return
  end
...
local function commit_rename()
  ...
  local _, err = craft.session.set_title({ id = id, title = title })
  if err then
    craft.ui.flash(err)
  else
...
local function open()
  ...
  craft.async.run(function()
    local stored, err = craft.session.list()
    if board ~= this_board then
      return
    end
    if err then
...
local last_status = {}
craft.api.create_autocmd("SessionStatusChanged", {
  callback = function(ev)
    ...
    if not d.focused then
      if d.status == "needs_input" then
        craft.ui.flash("\xe2\x97\x86 " .. d.title .. " needs input \xc2\xb7 /sessions")
      elseif d.status == "idle" and prev == "working" then
        craft.ui.flash("\xe2\x9c\x93 " .. d.title .. " finished \xc2\xb7 /sessions")
      end
    end
    if board then
      board.dirty = true
...
craft.api.register_command({
  ...
  handler = function(opts)
    ...
    local id, err = craft.session.current()
    if err then
      craft.ui.flash(err)
      return
    end
    local _, set_err = craft.session.set_title({ id = id, title = title })
    craft.ui.flash(set_err or ('Renamed to "' .. title .. '"'))
  end,
})
