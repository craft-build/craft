...
//! tokens that consumers (`craft-ui`, `craft-lua`) map to their own colours.
//!
...
use craft_highlight::{CodeHighlighter, SegmentColor, StyledSegment};
...
pub enum StyleToken {
    Text,
    InlineCode,
    /// Syntax-highlighted token. Carries the resolved color + modifiers so
    /// the consumer doesn't need to know the language.
    Highlight {
        fg: SegmentColor,
        bold: bool,
        italic: bool,
