Craft is an AI coding agent (like Claude Code and opencode), that is built bottom up to optimize costs and number of tokens used, while not sacrificing performance too much.
...
Homepage: ./site/. User docs: ./site/docs/. Most pages are generated by `craft-docgen/src/gen_*.rs`, run `just gen-docs` to regenerate.
