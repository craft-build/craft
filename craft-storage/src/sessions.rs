...
/// The one way costs are summed, re-exported by `craft-providers` so every
...
fn archive_if_shrinking<M, U, T>(dir: &Path, session: &Session<M, U, T>) {
    ...
    if let Err(e) = fs::create_dir_all(&archive_dir) {
        warn!(error = %e, session_id = %session.id.id(), "cannot create session archive dir");
        return;
    }
...
mod tests {
    ...
    use super::{
        ARCHIVE_DIR, ARCHIVE_KEEP, ARCHIVE_MAX_BYTES, CWD_INDEX_FILE, DEFAULT_TITLE, MAX_TITLE_LEN,
        MSG_PREFIX, SESSION_VERSION, StoredSubagent, generate_title, json_path, jsonl_path,
