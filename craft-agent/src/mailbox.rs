...
static MAILBOXES: LazyLock<Mutex<HashMap<CraftId, Weak<Mutex<State>>>>> =
...
pub struct MailboxError(CraftId);
...
impl SessionMailbox {
    pub fn register(session_id: CraftId) -> Self {
    ...
    pub fn notify(session_id: CraftId, text: String, wake: bool) -> Result<(), MailboxError> {
        let mailbox = {
            let mut mailboxes = lock(&MAILBOXES);
        ...
        if state.pending.len() == MAILBOX_CAPACITY {
            ...
            if let Some(dropped) = dropped {
                warn!(
                    ...
                    dropped_text = ?dropped.user_text(),
                    "session mailbox full; dropped the oldest observation",
                );
            }
        }
...
mod tests {
    ...
    fn legacy_and_canonical_ids_address_the_same_mailbox() {
        let legacy: CraftId = "01965087-4c71-7f00-8000-000000000001".parse().unwrap();
        let canonical: CraftId = legacy.to_string().parse().unwrap();
