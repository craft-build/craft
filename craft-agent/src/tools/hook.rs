...
//! knows: registry tools (native and Lua alike) and MCP tools.
//!
...
/// not told us it exercises none, only that nothing asks: `batch` and
...
pub enum Authority {
    ...
    /// Reach nobody declared: a tool naming no capability, or an MCP server
    /// tool. A layer needs every capability, because no narrower answer
    /// would be honest.
    Unbounded,
}
