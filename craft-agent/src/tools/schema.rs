...
/// Each call leaks a bounded amount (schema strings are typically <1KB). Leaked
/// memory accumulates if MCP servers reconnect or Lua plugins reload frequently,
/// but remains small relative to session lifetime.
