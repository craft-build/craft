...
pub async fn authenticate(
    ...
    let result = match interaction {
        Interaction::Cli => {
            ...
            tokio::select! {
                biased;
                result = callback.wait_for_callback(&state) => result,
                result = manual::wait_for_paste(&state) => result,
                _ = tokio::time::sleep(AUTH_TIMEOUT) => Err(format!(
        ...
        Interaction::Background => {
            ...
            match cause {
                Some(cause) => Err(format!("{cause}; run 'craft mcp auth {server_name}'")),
                None => tokio::select! {
                    biased;
                    result = callback.wait_for_callback(&state) => result,
                    _ = tokio::time::sleep(AUTH_TIMEOUT) => Err(format!(
