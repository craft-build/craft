...
use craft_storage::auth::load_mcp_auth;
...
impl McpTransport for HttpTransport {
    fn send_request<'a>(
        ...
        Box::pin(async move {
            ...
            loop {
                ...
                let http_req = self.build_request(
                    reqwest::Method::POST,
                    encode()?,
                    &negotiated,
                    auth.as_deref(),
    ...
    fn send_notification<'a>(
        ...
        Box::pin(async move {
            ...
            let http_req =
                self.build_request(reqwest::Method::POST, body, &negotiated, auth.as_deref());
...
mod tests {
    use super::*;
    use craft_storage::StateDir;
    use craft_storage::auth::{McpAuthData, OAuthTokens, save_mcp_auth};
    use serde_json::json;
    ...
    #[tokio::test]
    async fn server_negotiated_protocol_version_echoed_after_initialize() {
        let base = spawn_server(|_| {
            move |req: &Req| match req.protocol.as_deref() {
                None => (
                    200,
                    format!(
                        "{{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{{\"protocolVersion\":\"{NEGOTIATED_VERSION}\"}}}}"
                    ),
                ),
    ...
    #[tokio::test]
    async fn refreshes_token_and_retries_on_401() {
    ...
    #[tokio::test]
    async fn unauthorized_without_storage_fails_without_retry() {
    ...
    #[tokio::test]
    async fn config_authorization_header_is_sent() {
        let base = spawn_server(|_| {
            move |req: &Req| {
    ...
    #[tokio::test]
    async fn stored_token_injected_at_startup() {
        let tmp = tempfile::tempdir().unwrap();
        let storage = StateDir::from_path(tmp.path().to_path_buf());
