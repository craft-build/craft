...
fn discover_commands_inner(cwd: &Path, home: Option<&Path>) -> Vec<CustomCommand> {
    ...
    for dir in craft_storage::paths::user_config_dirs(home, "commands") {
...
mod tests {
    ...
    fn discover_project_overrides_global() {
        ...
        let cmd_dir = project.path().join(".craft/commands");
        ...
        let global_cmd_dir = global.path().join(".craft/commands");
