...
/// `craft.api.register_permission_rule`, keyed by plugin name. Shared between
...
mod tests {
    ...
    fn write_in_cwd() {
        ...
        assert!(matches!(
            default_mgr().check(&ToolKey::native("write"), &scope.to_string_lossy(), None),
    ...
    fn scope_matches_relative_pattern_before_dir_exists() {
        ...
        let value = cwd.join("__craft_nonexistent_dist/file.txt");
        assert!(!value.exists(), "test dir must not exist");
        assert!(
            scope_matches("__craft_nonexistent_dist/**", &value.to_string_lossy()),
    ...
    fn scope_matches_symlinked_parent_with_nonexistent_tail() {
        ...
        let tmp = std::env::temp_dir();
        let real = tmp.join("__craft_test_scope_symlink_tail_real");
        let link = tmp.join("__craft_test_scope_symlink_tail_link");
        let _ = std::fs::remove_dir_all(&real);
        let _ = std::fs::remove_file(&link);
        std::fs::create_dir_all(&real).unwrap();
        std::os::unix::fs::symlink(&real, &link).unwrap();
    ...
    fn boundary_nonexistent_cwd_proceeds_via_lexical_tail() {
        let missing = std::env::temp_dir().join("__craft_test_absent_cwd_xyz");
    ...
    fn command_matches_its_own_generalized_rule_edit_path() {
        ...
        let rule = &generalized_scopes(&ToolKey::native("edit"), &[scope.to_string()])[0];
        assert!(
            scope_matches(rule, &scope),
            "{scope:?} does not match its generalized rule {rule:?}"
        );
    ...
    fn plan_path_auto_allows_file_write_tools_only(tool: &str, expect_allowed: bool) {
        const PLAN_PATH: &str = "/home/user/.local/state/craft/plans/test.md";
        let plan_path = Path::new(PLAN_PATH);
        let mgr = default_mgr();
        assert_eq!(
            matches!(
                mgr.check(&ToolKey::native(tool), PLAN_PATH, Some(plan_path)),
                PermissionCheck::Allowed
            ),
            expect_allowed,
        );
    ...
    fn plan_path_multi_scope_all_must_match() {
        let plan = "/home/user/.local/state/craft/plans/test.md";
