...
impl Retrieve {
    ...
    pub async fn execute(&self, store: &SharedCompressionStore) -> Result<ToolOutput, String> {
        ...
        match guard.get(&self.hash) {
            ...
            None => Err(format!("no content found for hash={}. The original may have been evicted from the store.", self.hash)),
        }
    }
}
...
crate::tools::impl_tool!(
    Retrieve,
    audience = crate::tools::ToolAudience::MAIN
        | crate::tools::ToolAudience::RESEARCH_SUB,
);
...
impl crate::tools::ToolInvocation for Retrieve {
    fn start_header(&self) -> crate::tools::HeaderFuture {
        crate::tools::HeaderFuture::Ready(crate::tools::HeaderResult::plain(
            format!("retrieve {}", self.hash),
        ))
    }
    fn execute<'a>(self: Box<Self>, ctx: &'a crate::tools::ToolContext) -> crate::tools::ExecFuture<'a> {
