...
/// True if `msg` is an assistant message holding a prior VCC compaction summary.
...
/// Extract the text of a VCC summary message (its first `Text` block).
...
/// Whether the history's estimated tokens fit under the usable context window.
...
/// Returns `Ok(true)` if context is now under the limit, `Ok(false)` to fall back
/// to the LLM compactor.
pub(super) fn vcc_compact(
    history: &mut History,
    model: &Model,
    compaction_buffer: u32,
    multiplier: f64,
...
mod tests {
    ...
    async fn compact_replaces_history_with_summary() {
        let provider: std::sync::Arc<dyn Provider> =
            std::sync::Arc::new(MockProvider::new(vec![Ok(text_response(
                StopReason::EndTurn,
            ))]));
