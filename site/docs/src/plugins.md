...
- `craft.fn.jobfind(name)` finds the live job of this plugin registered under `name` by `jobstart`. Exited jobs never answer, so adopting by name restarts a job that died instead of latching onto a corpse; their `name` is still on the `joblist` row that explains why they died. Answers `(job_id, nil)` or `(nil, err)` when no live job holds the name.
