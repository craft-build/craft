...
fn span_style_to_lua(lua: &Lua, span: &craft_markdown::render::Span) -> LuaResult<mlua::Value> {
    use craft_markdown::render::StyleToken;
...
fn emphasis_style_name(e: craft_markdown::Emphasis) -> &'static str {
    if e.strike {
        "strikethrough"
...
fn markdown_lines_to_lua(lua: &Lua, lines: &[craft_markdown::render::Line]) -> LuaResult<Table> {
    let result = lua.create_table_with_capacity(lines.len(), 0)?;
...
mod tests {
    ...
    fn render_markdown(lua: &Lua, input: &str) -> Table {
        let lines = craft_markdown::render::render(input, MD_WIDTH);
    ...
    fn span_style_to_lua_highlight() {
        ...
        for (bold, italic, underline) in [(true, false, false), (true, true, true)] {
            let span = craft_markdown::render::Span {
                ...
                emphasis: craft_markdown::Emphasis::default(),
            };
