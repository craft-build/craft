...
pub(crate) struct AutocmdStore {
    ...
    pub(crate) pending_deletions: HashSet<u64>,
}
...
mod tests {
    ...
    fn register_and_remove() {
        ...
        let id = NEXT_AUTOCMD_ID.fetch_add(1, Ordering::Relaxed);
        store.register(id, "TurnEnd".into(), key, Arc::from("test"), false);
    ...
    fn clear_plugin_removes_only_matching() {
        ...
        let id1 = NEXT_AUTOCMD_ID.fetch_add(1, Ordering::Relaxed);
        let id2 = NEXT_AUTOCMD_ID.fetch_add(1, Ordering::Relaxed);
        store.register(id1, "TurnEnd".into(), k1, Arc::from("plugA"), false);
        store.register(id2, "TurnEnd".into(), k2, Arc::from("plugB"), false);
    ...
    #[test]
    fn remove_during_dispatch_tracks_pending_deletion() {
        let lua = Lua::new();
        let mut store = AutocmdStore::default();
        let f = lua.create_function(|_, ()| Ok(())).unwrap();
        let key = lua.create_registry_value(f).unwrap();
        ...
        let _entries = std::mem::take(store.listeners.get_mut("TurnEnd").unwrap());
        assert!(store.remove(id).is_empty());
        assert!(store.pending_deletions.contains(&id));
    ...
    fn once_flag_preserved() {
        ...
        let key = lua.create_registry_value(f).unwrap();
        let id = NEXT_AUTOCMD_ID.fetch_add(1, Ordering::Relaxed);
        store.register(id, "TurnEnd".into(), key, Arc::from("test"), true);
