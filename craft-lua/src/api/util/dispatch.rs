...
/// Tracks one seam's dispatch depth. The guard decrements on drop, so any
/// unwinding path can never leave the depth stuck high.
...
mod tests {
    ...
    #[tokio::test]
    async fn call_swallowing_hides_errors_but_not_values() {
        let lua = Lua::new();
