...
pub enum SessionRequest {
    ...
    Live,
    Current,
    /// UI-mode fallback for `craft.session.read` when no snapshot slot is
...
/// UI; `craft.fn.winsaveview` is what puts it in Vim's 1-based shape.
...
mod tests {
    ...
    fn ui_roundtrip_fails_once_the_loop_detaches() {
        ...
        attachment.detach();
        let detached = tokio::runtime::Builder::new_current_thread()
            .build()
            .unwrap()
            .block_on(ui_roundtrip(Some(&tx), |reply_tx| UiAction::WinSaveView {
                reply_tx,
            }));
