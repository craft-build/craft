...
/// The user's `plugins.<name>` table from `craft.setup`, passed through opaquely.
...
/// by docgen and by the loader to reject `plugins.<name>` keys no plugin
/// declared.
...
/// Register the `craft.api.register_options` function bound to `plugin`
/// reading from `opts`. The returned table is the merged options: user value
/// where set, otherwise the declared default.
pub(crate) fn create_register_options_fn(
    lua: &Lua,
    plugin: Arc<str>,
    opts: PluginOpts,
