...
fn finish(spec: Spec) -> LuaResult<Spec> {
    if craft_pack::git::http_source_has_userinfo(&spec.src) {
        let source = crate::pack::sanitize_message(&craft_pack::git::redact(&spec.src));
        return Err(mlua::Error::runtime(format!(
            "pack.add: source {source:?} contains HTTP credentials; use a Git credential helper instead"
        )));
    }
    if !craft_pack::name_is_safe(&spec.name) {
        let source = crate::pack::sanitize_message(&craft_pack::git::redact(&spec.src));
        return Err(mlua::Error::runtime(format!(
...
///
/// `specs` are sources or tables with `src`, `name`, `version`, and `data`.
/// `opts.confirm` controls source confirmation. `opts.load` is a boolean or a
    ...
    for declared in parsed {
        ...
        match existing {
            Some(existing) if existing.spec != declared.spec => {
                let source =
                    crate::pack::sanitize_message(&craft_pack::git::redact(&existing.spec.src));
                return Err(mlua::Error::runtime(format!(
...
fn enqueue(lua: &Lua, name: String) -> LuaResult<()> {
    if !craft_pack::name_is_safe(&name) {
        return Err(mlua::Error::runtime(format!(
    ...
    let mut declarations = store.lock().expect("pack declarations");
    if declarations.drained {
        return Err(mlua::Error::runtime(format!("craft.packadd: {AFTER_LOAD}")));
    }
...
pub(crate) fn add_packadd(lua: &Lua, craft: &Table) -> LuaResult<()> {
    craft.set("packadd", lua.create_function(packadd)?)
}

/// Get package state without changing the installed set.
///
/// `names` selects package names; omit it for all managed packages. `opts` is
/// reserved. Returns package records with `spec`, `path`, `rev`, and `active`.
...
fn package_state_table(
    ...
    let manager = site.map(craft_pack::manager::Manager::new);
...
pub(crate) fn spec_to_lua(
    ...
    table.set("src", craft_pack::git::redact(&spec.src))?;
    ...
    table.set(
        ...
        lua.create_function(|_, _: MultiValue| -> LuaResult<()> {
            Err(mlua::Error::runtime(
                "craft.pack.add is only available in the global init.lua",
            ))
        })?,
    )?;
    table.set(
...
/// The table the global `init.lua` sees: `add` declares packages, `get`
/// inspects state without changing the installed set.
pub(crate) fn create_pack_table(lua: &Lua) -> LuaResult<Table> {
    let table = create_pack_read_table(lua)?;
    table.set(
        "add",
        lua.create_function(|lua, (specs, opts): (Table, Option<Table>)| add(lua, specs, opts))?,
    )?;
    Ok(table)
}
...
mod tests {
    ...
    fn packadd_queues_one_activation() {
        ...
        add_packadd(&lua, &craft).unwrap();
        let packadd: mlua::Function = craft.get("packadd").unwrap();
    ...
    fn get_preserves_order_and_reports_exact_state() {
        ...
        for name in ["alpha", "legacy"] {
            std::fs::create_dir_all(craft_pack::paths::revision_dir(site.path(), name, TEST_REV))
                .unwrap();
        }
        ...
        assert_eq!(
            ...
            craft_pack::paths::revision_dir(site.path(), "alpha", TEST_REV)
                .display()
                .to_string()
        );
