//! `craft.packadd`, the activation path for a package under `pack/*/opt/`.
...
/// Registers `craft.packadd` on the root table.
///
...
fn enqueue(lua: &Lua, op: PackOp) -> LuaResult<()> {
    ...
    if declarations.drained {
        return Err(mlua::Error::runtime(
            "craft.packadd: packages have already been loaded, so it only works \
...
mod tests {
    ...
    fn packadd_records_each_named_package_once() {
        ...
        let f: mlua::Function = craft.get("packadd").expect("packadd is on the root table");
        f.call::<()>("goal").unwrap();
    ...
    fn packadd_without_a_store_reports_rather_than_panicking() {
        ...
        let craft = lua.create_table().unwrap();
        add_packadd(&lua, &craft).unwrap();
        let f: mlua::Function = craft.get("packadd").unwrap();
