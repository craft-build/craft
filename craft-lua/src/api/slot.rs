...
use craft_agent::tools::HookStage;
...
///
/// A layer is handed a `prev` closure whose single-use state records how far
/// the chain got, so a layer that throws after calling `prev` cannot make the
/// rest run twice and one that throws before cannot make it run zero times.
/// The layer list is a snapshot, taken under the app-data lock, so an unload
/// cannot race a call: all Lua runs on the runtime thread and unloads arrive
...
///   layer's own input
/// - called `prev` and it finished: use the recorded result
/// - called `prev`, which is still unwinding: report the inconsistency
///
...
pub(crate) fn add_slot_methods(api_table: &Table, lua: &Lua, plugin: Arc<str>) -> LuaResult<()> {
    ...
    // Create a named extension point owned by your plugin. You provide the
    // default, anyone else can wrap it with `set_slot`. The returned callable
    // runs the full chain: outermost layer first, then inward, ending at the
    // default.
    //
    // Throws if another plugin already owns a slot with the same name, or if
    // the name starts with "tool.", which the host fires itself.
    //
    // The chain is async: the default and every layer may park (`craft.fs.*`,
    // `craft.fn.jobwait`, ...), and so does the returned callable. Call it
    // from a tool handler, a command, or an autocmd, rather than from a
    // `header` or `restore` function, which cannot wait. The chain runs in
    // your task, so cancelling the caller cancels the layers it is waiting on.
    api_table.set(
        "declare_slot",
        lua.create_function(move |lua, (name, default): (String, Function)| {
            if name.starts_with(HOST_PREFIX) {
                return Err(mlua::Error::runtime(format!(
                    "slot '{name}' is host owned: '{HOST_PREFIX}' names are fired by craft itself, \
    ...
    let p = Arc::clone(&plugin);
    // Wrap a slot with a layer. A layer may park, and one that throws is
    // skipped: the chain continues as if it had returned `prev(...)`
    // untouched, so a broken layer never takes the seam down with it.
    //
    // Layers wrap in registration order, so the last one registered runs
    // first and sees the value before the others do.
    //
    // Craft fires two slots per tool itself: `tool.<name>.input` before
    ...
    // table to replace the value, nothing to leave it alone, or `nil, reason`
    // to stop the call. Wrapping one costs the capability the tool declares,
    // and a tool declaring none costs every permission.
    api_table.set(
        "set_slot",
