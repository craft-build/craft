...
use craft_agent::tools::hook::{Authority, Verdict};
...
use include_dir::Dir;
use mlua::{
    Function, Lua, MultiValue, RegistryKey, Table, Value as LuaValue, chunk::Chunk,
    chunk::ChunkMode, chunk::Compiler, ffi,
};
...
pub enum Request {
    ...
    /// one so `craft.session.read` works without a UI to ask.
...
pub(crate) struct TaskCell {
    /// Distinguishes concurrent tasks, for re-entrancy bounds that count per
    /// task (slot chains) rather than per VM.
    pub(crate) id: u64,
    ...
    /// `Arc` so the listener can outlive the cell lock it was born under.
    pub(crate) deadline_changed: Arc<tokio::sync::Notify>,
    ...
    /// `craft.async.on_cancel` callbacks, fired once by [`ScopedFuture::poll`]
...
/// Backs `craft.async.on_cancel`. An already cancelled task has no
...
impl<F: Future> Future for ScopedFuture<F> {
    ...
    fn poll(
        ...
        let cancel_fired = match this.cancel_wait.as_mut() {
            Some(wait) => wait.as_mut().poll(cx).is_ready(),
            None => false,
        };
        if cancel_fired {
...
/// to interrupt and renews its grace at every yield. Biased so a handler that
/// finished in the same slice still has its result reported, even one whose
/// deadline had already lapsed when it started.
async fn until_abandoned<T>(
    ...
    tokio::select! {
        ...
        msg = async {
            loop {
                // Register before reading: a `ctx:set_deadline` landing between
                // the two wakes us instead of leaving us armed on the stale
                // deadline.
                let notify = Arc::clone(&lock_cell(handle).deadline_changed);
                let mut changed = std::pin::pin!(notify.notified_owned());
                if changed.as_mut().enable() {
                    continue;
                }
                ...
                match deadline {
                    ...
                    Some(dl) => {
                        tokio::select! {
                            _ = tokio::time::sleep_until(tokio::time::Instant::from_std(dl)) => break,
                            _ = changed.as_mut() => {}
                        }
                    }
                    None => changed.await,
            ...
            for task in tasks {
                if !task.cancel.is_cancelled() {
                    ...
                    let handle = TaskCell::new(
                        ...
                        None,
                        active_terminal_backend(&self.lua),
                    )
                    .into_handle();
                    if let Err(e) = scope
                        .scope_future(run_work_fn(&self.lua, &task.work_fn, &handle))
                        .await
                    {
...
/// for those: the bash plugin's `restore` parses it to re-render the timeout
/// sentinel on session reload.
...
async fn run_tool_call(
    ...
    let call_future = scope.scope_future(async {
            ...
            .unwrap_or_else(|msg| Err(mlua::Error::runtime(msg)))
        {
            ...
            // Bound before the `and_then` so the guard drops before the hooks
            // run: they lock the same cell.
            Err(e) => {
                let kill = lock_cell(&handle).doomed(Instant::now());
...
mod tests {
    ...
    /// Polls a future once with a no-op waker. The cancel hook tests use it to
    /// drive [`ScopedFuture`] exactly one step: a scope whose token is already
    /// tripped fires its hooks on that single poll.
    ...
    fn poll_cancelled_scope_once(scope: &TaskScope) {
        let mut fut = Box::pin(scope.scope_future(std::future::pending::<()>()));
        assert!(poll_once(fut.as_mut()).is_pending());
    }

    fn block_on_or_fail<T>(fut: impl Future<Output = T>) -> T {
        ...
        tokio::task::LocalSet::new().block_on(&rt, async {
            tokio::select! {
                biased;
                out = fut => out,
                _ = tokio::time::sleep(TEST_WAKE_TIMEOUT) => panic!("{TEST_WAKE_TIMEOUT_MSG}"),
            }
        })
    }
    ...
    fn cancel_hook_fires_once_across_nested_scopes_and_repeated_polls() {
        ...
        let mut nested =
            Box::pin(scope.scope_future(scope.scope_future(std::future::pending::<()>())));
        for _ in 0..HOOK_POLL_ROUNDS {
            assert!(poll_once(nested.as_mut()).is_pending());
        }
    ...
    #[test_case(true ; "after_firing")]
    #[test_case(false ; "when_an_uncancelled_scope_ends")]
    ...
    fn cancel_hook_fires_on_a_parked_task_and_leaves_it_wakeable() {
        ...
        let out = block_on_or_fail(async {
            tokio::select! {
                biased;
                out = scope.scope_future(async move { item_rx.recv_async().await.unwrap() }) => out,
                _ = async move {
                    trigger.cancel();
    ...
    fn cancel_hooks_fire_before_an_inner_future_that_is_ready_at_once() {
        ...
        let out = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap()
            .block_on(scope.scope_future(poll_fn(move |_| {
    ...
    /// A `craft.async.run` task gets its own cell but inherits the parent's
    ...
    /// bufs it never owned.
    #[test]
    fn cancel_hook_in_a_child_scope_fires_on_the_inherited_cancel() {
        let lua = Lua::new();
        let (trigger, parent) = live_scope(&lua);
        let (fired_tx, fired_rx) = flume::unbounded();
        recording_hook(&lua, &fired_tx, HOOK_PARENT_MARK);

        let parent_cancel = lock_cell(parent.handle()).cancel.clone();
        let child = TaskScope::new(
            &lua,
            TaskCell::new(
                parent_cancel,
                None,
        ...
        recording_hook(&lua, &fired_tx, HOOK_CHILD_MARK);

        trigger.cancel();
        poll_cancelled_scope_once(&child);
        assert_eq!(
            fired_rx.try_iter().collect::<Vec<_>>(),
            [HOOK_CHILD_MARK],
            "the child's hook must fire on the inherited cancel, not the parent's"
        );
    ...
    async fn until_abandoned_ends_a_parked_handler_only_after_its_window() {
        ...
        let early = tokio::time::timeout(
            ...
            until_abandoned(parked(), &task_handle(cancelled_token(), None)),
        )
        .await;
        assert!(
            early.is_err(),
    ...
    #[tokio::test]
    async fn until_abandoned_re_arms_on_a_deadline_set_after_it_started() {
