//! The bridge between [`craft_agent::tools::hook`] and the slot chains plugins
//! register. Everything here is a hop: dispatch asks, the Lua thread answers,
//! and nothing decides anything on the way.

use std::sync::Arc;

use craft_agent::tools::hook::{HookCall, HookStage, ToolHook, Verdict};
use flume::Sender;
...
impl ToolHook for SlotHook {
    ...
    fn run<'a>(
        ...
        value: Value,
        call: &'a HookCall<'a>,
    ) -> craft_agent::tools::BoxFuture<'a, Verdict> {
        let (reply, answer) = flume::bounded(1);
        let request = Request::RunToolHook {
            run: HookRun {
