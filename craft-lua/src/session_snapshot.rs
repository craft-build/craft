//! The one shape `craft.session.read()` returns. The UI and the headless
...
/// Backs `craft.session.read` for the single session headless drivers
/// (`craft -p`, sdk mode). The driver writes while the Lua thread reads, hence
...
impl HeadlessSnapshot {
    ...
    /// Install as the provider `craft.session.read` answers from. `mode`
    ...
    pub fn install(
        ...
        handle.install_session_snapshot(Box::new(move |id| {
            ...
            serde_json::to_value(SessionSnapshot {
                ...
                focused: true,
                updated_at: craft_storage::now_epoch(),
                queue: None,
                usage: totals.usage,
...
#[cfg(test)]
mod tests {
    use craft_agent::{DoneReason, TurnCompleteEvent};
