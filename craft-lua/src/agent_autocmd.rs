...
//! `craft -p` and sdk mode, so a plugin sees the same events whichever way craft
//! was started. `craft-acp` drives the agent from its own loop and does not call
...
/// `AwaitingGoalApproval` is the flow pipeline's pause, not a turn end either.
fn turn_end_reason(reason: DoneReason) -> Option<&'static str> {
    match reason {
        ...
        DoneReason::Compact | DoneReason::AwaitingGoalApproval => None,
    }
}

#[cfg(test)]
mod tests {
    use craft_agent::{ToolDoneEvent, ToolOutput, ToolStartEvent};
    ...
    #[test_case(DoneReason::AwaitingGoalApproval, None ; "goal_approval_pause_is_not_a_turn")]
