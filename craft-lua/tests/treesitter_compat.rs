...
local parser = craft.treesitter.get_parser(source, "rust")
...
local query = craft.treesitter.query.parse("rust", [[
...
for id, node, metadata in query:iter_captures(root, source) do
    ...
    local text = craft.treesitter.get_node_text(node, source)
...
local query = craft.treesitter.query.parse("rust", "(function_item) @fn")
...
local id_query = craft.treesitter.query.parse("rust", "(identifier) @id")
...
local gnr_sr, gnr_sc, gnr_er, gnr_ec = craft.treesitter.get_node_range(root)
assert(gnr_sr == sr and gnr_sc == sc)

local range6 = craft.treesitter.get_range(root)
assert(range6[1] == sr and range6[3] == sb)

assert(craft.treesitter.is_in_node_range(root, 1, 0) == true)

assert(craft.treesitter.node_contains(root, {{1, 0, 2, 0}}) == true)

local child = root:named_child(0)
assert(child ~= nil)
assert(craft.treesitter.is_ancestor(root, child) == true)
assert(craft.treesitter.is_ancestor(child, root) == false)

local text = craft.treesitter.get_node_text(child, source)
...
fn language_module() {
    ...
    run_lua(
        ...
        "language_mod",
        r#"
craft.treesitter.language.add("rust")

craft.treesitter.language.register("rust", "rs")
assert(craft.treesitter.language.get_lang("rs") == "rust")

craft.treesitter.language.register("python", {"py", "pyi"})
assert(craft.treesitter.language.get_lang("py") == "python")
assert(craft.treesitter.language.get_lang("pyi") == "python")

local fts = craft.treesitter.language.get_filetypes("python")
assert(#fts == 2, "expected 2 filetypes for python")

assert(craft.treesitter.language.get_lang("unknown_ext_xyz") == nil)

local info = craft.treesitter.language.inspect("rust")
...
local parser = craft.treesitter.get_parser(source, "python")
local root = parser:parse()[1]:root()
assert(root:type() == "module")

local query = craft.treesitter.query.parse("python", [[
...
local parser = craft.treesitter.get_parser(source, "typescript")
local root = parser:parse()[1]:root()
assert(root:type() == "program")

local query = craft.treesitter.query.parse("typescript", [[
