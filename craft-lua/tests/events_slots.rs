...
use craft_agent::tools::hook::{self, Authority, HookCall, HookStage, Verdict};
use craft_lua::test_support::MAX_HOOK_DEPTH;
use craft_lua::{Permission, PluginHost, PluginPermissions, SessionEndReason};
...
/// about.
fn call_of<'a>(tool: &'a str, authority: Authority, cancel: &'a CancelToken) -> HookCall<'a> {
    HookCall {
        tool,
        tool_id: TOOL_ID,
        session_id: None,
...
/// none, the way an MCP tool is priced.
async fn fire(
    reg: &ToolRegistry,
    tool: &str,
    stage: HookStage,
    ...
    fire_call(
        reg,
        &call_of(tool, authority, &CancelToken::none()),
        stage,
        value,
    )
    .await
...
async fn input(reg: &ToolRegistry, tool: &str, command: &str) -> (Option<String>, Option<String>) {
    match fire(
        reg,
        tool,
        HookStage::Input,
...
async fn output(
    ...
    match fire(reg, tool, HookStage::Output, value).await {
...
#[tokio::test]
async fn tool_input_slot_rewrites_denies_and_passes_through() {
    let (reg, host) = host();
    slotted_tool(&host);
    load(
        &host,
        "policy",
        r#"
craft.api.set_slot("tool.slotted.input", function(prev, input, ctx)
...
"#,
    ...
    assert_eq!(
        input(&reg, SLOT_TOOL, "grep -r x .").await,
        (Some("rg x".to_owned()), None)
    );
    assert_eq!(
        input(&reg, SLOT_TOOL, "denied").await,
        (None, Some("use rg".to_owned()))
    );
    assert_eq!(
        input(&reg, SLOT_TOOL, "ls").await,
        (None, None),
...
fn layer(tool: &str, stage: HookStage, body: &str) -> String {
    format!(
        r#"craft.api.set_slot("tool.{tool}.{}", function(prev, value, ctx) {body} end)"#,
        stage.as_str()
    )
}
...
/// that one. A tool declaring none has said nothing about how far it reaches,
/// so layering it costs every capability.
///
...
#[tokio::test]
async fn a_layer_pays_the_authority_of_the_call_it_filters(
    tool: &str,
    ...
    assert_eq!(input(&reg, tool, COMMAND).await, (expected, None));
}

#[tokio::test]
async fn tool_slot_with_no_layers_never_reaches_lua() {
    let (reg, host) = host();
    slotted_tool(&host);
    assert_eq!(input(&reg, SLOT_TOOL, COMMAND).await, PASS_THROUGH);
    assert_eq!(output(&reg, SLOT_TOOL, "out", false).await, None);
}

/// The chain owns a task, which is what delivers job events to a layer waiting
/// on one.
#[tokio::test]
async fn tool_input_layer_may_run_a_job() {
    let (reg, host) = host();
    slotted_tool(&host);
    load(
        &host,
        ...
        r#"
craft.api.set_slot("tool.slotted.input", function(prev, input, ctx)
    local result = craft.fn.jobwait(craft.fn.jobstart({ "echo", "from-job" }))
...
#[tokio::test]
async fn tool_output_slot_rewrites_text_and_error_flag() {
    let (reg, host) = host();
    slotted_tool(&host);
    load(
        &host,
        "trimmer",
        r#"
craft.api.set_slot("tool.slotted.output", function(prev, out, ctx)
...
"#,
    ...
    assert_eq!(
        output(&reg, SLOT_TOOL, "0123456789", false).await,
        Some(("012".to_owned(), true))
    );
}

#[tokio::test]
async fn unloading_the_layer_owner_restores_the_fast_path() {
    let (reg, host) = host();
    slotted_tool(&host);
    load(&host, "temporary", &hijack_layer(SLOT_TOOL));
    assert_eq!(
        input(&reg, SLOT_TOOL, COMMAND).await,
...
#[tokio::test]
async fn overlapping_chains_all_run() {
    const CONCURRENT: usize = MAX_HOOK_DEPTH as usize * 2;
    let (reg, host) = host();
    slotted_tool(&host);
...
"#
    ...
    let call = call_of(SLOT_TOOL, Authority::Unbounded, &cancel);

    // Joined, so the chains are genuinely in flight at once. Awaiting them one
    // at a time would never overlap and never notice.
    let mut slots: Vec<Option<Verdict>> = (0..CONCURRENT).map(|_| None).collect();
    let mut futs: Vec<std::pin::Pin<Box<dyn Future<Output = Verdict> + Send + '_>>> = Vec::new();
    for i in 0..CONCURRENT {
        let value = serde_json::json!({ COMMAND_FIELD: i.to_string() });
        futs.push(Box::pin(hook.run(HookStage::Input, value, &call)));
    }
    let verdicts = within(std::future::poll_fn(move |cx| {
        let mut pending = false;
        for (fut, slot) in futs.iter_mut().zip(slots.iter_mut()) {
            if slot.is_none() {
...
#[tokio::test]
async fn a_pass_through_layer_leaves_the_input_untouched(value: serde_json::Value) {
    let (reg, host) = host();
    slotted_tool(&host);
    load(
        &host,
    ...
    let verdict = fire(&reg, SLOT_TOOL, HookStage::Input, value).await;
...
#[tokio::test]
async fn cancelling_the_call_kills_the_chain() {
    let (reg, host) = host();
    slotted_tool(&host);
    load(
        &host,
    ...
    let verdict = fire_call(&reg, &call, HookStage::Input, value).await;
...
#[tokio::test]
async fn a_parked_layer_ends_at_the_window_it_was_given() {
    let (reg, host) = host();
    slotted_tool(&host);
    load(
        &host,
        ...
        &layer(
            ...
            HookStage::Input,
            &format!(
                r#"craft.fn.jobwait(craft.fn.jobstart({{ "sleep", "{}" }}), {})
                value.{COMMAND_FIELD} = "{HIJACKED}"
                return prev(value, ctx)"#,
                PARKED_FOR.as_secs(),
    ...
    let cancel = CancelToken::none();
    let mut call = call_of(SLOT_TOOL, Authority::Unbounded, &cancel);
...
#[tokio::test]
async fn host_slot_names_are_reserved() {
    let (_reg, host) = host();
    let err = host
        .load_source(
            "squatter",
            r#"craft.api.declare_slot("tool.bash.input", function(i) return i end)"#,
        )
...
/// call it already had rather than hand the tool a shape nobody promised.
...
#[tokio::test]
async fn a_malformed_layer_answer_leaves_the_call_alone(answer: &str) {
    let (reg, host) = host();
    slotted_tool(&host);
    load(
        &host,
...
#[tokio::test]
async fn an_output_layer_stops_the_call_with_its_reason() {
    ...
    assert_eq!(
        output(&reg, SLOT_TOOL, "secret", false).await,
...
#[tokio::test]
async fn a_broken_layer_is_skipped_and_the_chain_still_answers() {
    let (reg, host) = host();
    slotted_tool(&host);
...
#[tokio::test]
async fn layers_compose_with_the_last_registered_outermost() {
    let (reg, host) = host();
    slotted_tool(&host);
...
#[tokio::test]
async fn only_the_entitled_layer_of_two_runs() {
    let (reg, host) = host();
    guarded_tool(&host);
    ...
    assert_eq!(
        input(&reg, GUARDED_TOOL, COMMAND).await,
...
#[tokio::test]
async fn a_layer_that_never_calls_prev_short_circuits() {
...
#[tokio::test]
async fn a_reload_that_narrows_permissions_applies_to_the_next_call() {
    let (reg, host) = host();
    guarded_tool(&host);
...
#[tokio::test]
async fn a_dropped_host_leaves_the_call_unchanged() {
    let (reg, host) = host();
    slotted_tool(&host);
    ...
    let call = call_of(SLOT_TOOL, Authority::Unbounded, &cancel);
    let value = serde_json::json!({ COMMAND_FIELD: COMMAND });
    let verdict = within(hook.run(HookStage::Input, value, &call)).await;
...
#[tokio::test]
async fn a_tool_slot_that_names_no_stage_never_fires() {
    let (reg, host) = host();
    slotted_tool(&host);
    load(
        &host,
        LAYER_PLUGIN,
        &format!(
            r#"craft.api.set_slot("tool.{SLOT_TOOL}.header", function(prev, value, ctx)
                value.{COMMAND_FIELD} = "{HIJACKED}"
                return prev(value, ctx)
            end)"#
        ),
...
#[tokio::test]
async fn slot_chain_may_suspend() {
    let (_reg, host) = host();
    load(
        &host,
        "slot_suspends",
        r#"
local d = craft.api.declare_slot("sd", function()
    local _, err = craft.fs.read("/nope")
    return err ~= nil
end)
assert(d() == true, "a parking default reaches the filesystem and comes back")

local l = craft.api.declare_slot("sl", function(x) return x end)
craft.api.set_slot("sl", function(prev, x)
    local _, err = craft.fs.read("/nope")
