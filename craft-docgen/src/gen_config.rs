...
fn write_theme_section(out: &mut String) {
    ...
    writeln!(
        ...
        "You can add your own themes too. Drop a `<name>.toml` file into \
         `themes/` inside your Craft config directory, for example \
         `~/.config/craft/themes/`. If it reuses a built-in name, yours wins.\n"
    )
    .unwrap();
    writeln!(
        out,
        "Themes use 24-bit colors by default, but not every terminal can show \
         them. Craft checks the environment, terminfo, and the terminal itself, \
    ...
    writeln!(
        ...
        "Theme files can also name terminal colors instead of giving hex \
         ...
         `default` means the terminal default. Craft also takes a number from \
    ...
    writeln!(
        ...
        "These work everywhere a hex value does, including syntax \
         ...
         palette your terminal already defines. Craft passes them through as \
