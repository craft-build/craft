...
use craft_agent::tools::{DescriptionContext, ToolFilter, ToolRegistry, ToolSource};
use craft_config::OPT_IN_TOOLS;
...
/// `bash` interpolates `craft.uv.cwd()` and `websearch` interpolates
...
fn collect_tool_info(
    ...
    entry: &craft_agent::tools::RegisteredTool,
) -> Option<ToolInfo> {
    let name = entry.name();
